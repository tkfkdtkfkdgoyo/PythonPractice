def load_file(): # 파일을 읽어서 리스트로 반환
    ...
    except FileNotFoundError: # 첫번째 에러 처리 방법
...
def add(students):
    ...
    except ValueError: # 두번째 에러 처리 방법
        print('정수를 입력해주세요.')
        return
...
def search(students):
    ...
    except KeyError: # 세번째 에러 처리 방법
        print('잘못된 key입니다. name, age, score 중 하나를 입력해주세요.')
...
def delete(students):
    ...
    try:
        if len(students) == 0:
            raise IndexError
    except IndexError: # 네번째 에러 처리 방법
        print('데이터가 없습니다.')
        return
